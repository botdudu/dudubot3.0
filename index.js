...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["031986283462@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falha ao converteri ${tipe} para o adesivo`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
				...
				case 'url2img':
					...
					if (!tipelist.includes(args[0])) return reply('desktop ipe|tablet|celular')
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('Modo Simi está ativado')
						samih.push(from)
