...
const fs = require('fs')
const moment = require('fuso horário')
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["3186283462@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falhou, no momento da conversão ${tipe} para adesivos`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
				...
				case 'tiktokstalk':
					try {
						...
						reply('O nome de usuário pode ser inválido')
					}
					break
				case 'nulis':
				case 'tulis':
				...
				case 'url2img':
					...
					if (args.length < 1) return reply('Que tipo um?')
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('O que você quer adicionar um gênio??')
				...
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) resposta de retorno('❌ reply stickernya um ❌')
					reply(mess.wait)
